...
export const Tooltip = ({ children, title, placement = 'top-center', ...rest }: TooltipProps) => {
  return (
    <TooltipRadix.Provider {...rest}>
      <TooltipRadix.Root>
        <TooltipRadix.Trigger asChild>{children}</TooltipRadix.Trigger>
        <TooltipRadix.Content {...direction[placement]} className="popup--content tooltip" sideOffset={5}>
          {title}
