...
export const Popover = ({ trigger, children, placement = 'top-start' }: PopoverRootProps) => (
  <PopoverRadix.Root>
    <PopoverRadix.Trigger asChild>{trigger}</PopoverRadix.Trigger>
    <PopoverRadix.Portal>
      <PopoverRadix.Content {...direction[placement]} className="popup--content" sideOffset={5}>
        <PopoverRadix.Arrow className="popup--arrow" />
        <div className="u_flex u_justify-end">
          <PopoverRadix.Close asChild aria-label="close">
            <IconButton className="popup--close" icon={<Cross width={16} />} title="Close" size="sm" />
          </PopoverRadix.Close>
        </div>
        {children}
